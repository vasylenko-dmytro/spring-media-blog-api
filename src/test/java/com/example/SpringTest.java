...
public class SpringTest {
    ...
    /**
     * After retrieving the AccountRepository bean, it should exhibit the functionality
     * of a JPARepository for an "Account" entity.
     */
    @Test
    public void accountRepositoryIsRepositoryTest() throws ReflectiveOperationException {
        ...
        if (saveMethod == null || findAllMethod == null) {
            fail("The save / findAll methods were not found. "
                    + "Ensure that AccountRepository properly  extends JPARepository.");
        }
        List<Account> accountList1 = (List<Account>) findAllMethod.invoke(repository, new Object[]{});
        System.out.println(accountList1);
        assertEquals(4, accountList1.size(), "There should be no accounts in the JPARepository on startup.");
        Account actualAccount = (Account) saveMethod.invoke(repository, testAccount);
        assertEquals(actualAccount.getUsername(), expectedUsername);
        List<Account> accountList2 = (List<Account>) findAllMethod.invoke(repository, new Object[]{});
        assertTrue(accountList2.size() > 4, "The account should be addable to the  JPARepository.");
    }

    /**
     * After retrieving the MessageRepository bean, it should exhibit the functionality
     * of a JPARepository for a "Message" entity.
     */
    @Test
    public void messageRepositoryIsRepositoryTest() throws ReflectiveOperationException {
        ...
        if (saveMethod == null || findAllMethod == null) {
            fail("The save / findAll methods were not found. Ensure that MessageRepository properly " +
                    "extends JPARepository.");
        }
        List<Account> accountList1 = (List<Account>) findAllMethod.invoke(repository, new Object[]{});
        System.out.println(accountList1);
        assertEquals(3, accountList1.size(), "There should be no messages in the JPARepository on startup.");
        Message actualMessage = (Message) saveMethod.invoke(repository, testMessage);
        assertEquals(actualMessage.getMessage_text(), expectedText);
        List<Account> accountList2 = (List<Account>) findAllMethod.invoke(repository, new Object[]{});
        assertTrue(accountList2.size() > 3, "The message should be addable to the  JPARepository.");
    }

    /**
     * Verify the functionality of Spring MVC, independently of the project requirement endpoints,
     * by sending a request to an arbitrary endpoint.
     */
    @Test
